...
const Template: ComponentStory<typeof SearchFormField> = (args) => {
  const [chosenCurrency, setChosenCurrency] = useState<ICurrency>(CurrenciesMock[0]);
  ...
  const getSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (e.target.value !== '') {
      ...
      setFilteredCurrencies([CurrenciesMock[2]].filter(item =>
        pattern.test(item.name.toLowerCase()) || pattern.test(item.ticker.toLowerCase())
      ));
      setIsFiltered(true);
    } else {
      setIsFiltered(false);
  ...
  const onClick = (e: React.MouseEvent, value: ICurrency) => {
